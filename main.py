...
if os.path.exists(CAMINHO_JSON_PADRAO):
    try:
        ...
        limites_df["Date"] = limites_df["Timestamp"].dt.date
        ...
        st.session_state.limites_por_medidor_horario = {
            medidor: list(
                limites_df[limites_df["Date"] == limites_df["Date"].min()].sort_values("Hora")[medidor].values)
            for medidor in limites_df.columns
            if medidor not in ["Timestamp", "Date", "Hora"]
        }
...
with st.sidebar:
    ...
    origem_dados = st.radio("Choose the source of the data:", ["Google Sheets", "Paste manually"])
    ...
    if origem_dados == "Google Sheets":
        ...
        if not df_temp.empty:
            ...
            st.sidebar.warning("It was not possible to determine the last update date.")
    else:
    ...
    if st.button("✉️ Send by email", key="enviar_email_sidebar", use_container_width=True):
        if not to_email:
            ...
            try:
                ...
                msg["From"] = EMAIL
                msg["To"] = to_email
                msg["Subject"] = "Energy Consumption Report"
                ...
                # Corpo do e-mail
                body = f"""
                Summary of the Day {st.session_state.data_selecionada.strftime('%d/%m/%Y')}:

                General Consumption: {st.session_state.consumo_geral:.2f} kWh
                General Limit: {st.session_state.limite_geral:.2f} kWh
                Balance for the Day (Overall): {st.session_state.saldo_geral:.2f} kWh

                Consumption of the Productive Area: {st.session_state.consumo_area:.2f} kWh
                Productive Area Limit:{st.session_state.limites_area:.2f} kWh
                Daily Balance (Production Area): {st.session_state.saldo_area:.2f} kWh
                """

                # Adiciona alerta se houver medidores excedidos
                if medidores_excedidos:
                    body += "\n⚠️ Alert: The following meters have exceeded their daily limits:\n"
    ...
    try:
        with st.spinner("Processing data..."):
            ...
            if "limites_df" in st.session_state:
                ...
                limites_dia_df = limites_df[limites_df["Date"] == data_selecionada]
                st.session_state.limites_por_medidor_horario = {
            ...
            with tabs[0]:
                ...
                consumo_diario["Date"] = consumo_diario["Datetime"].dt.date
                consumo_agrupado = consumo_diario.groupby("Date")[medidores_disponiveis].sum().reset_index()
                ...
                df_consumo["Date"] = df_consumo["Datetime"].dt.date

                # Contar quantas diferenças por dia (consumos horários)
                contagem_por_dia = df_consumo.groupby("Date").size()

                # Considerar apenas dias com 24 diferenças (ou seja, 25 leituras)
                dias_completos = contagem_por_dia[contagem_por_dia == 24].index
                df_filtrado = df_consumo[df_consumo["Date"].isin(dias_completos)]

                # Agregar consumo diário
                df_diario = df_filtrado.groupby("Date")[colunas_medidores].sum().reset_index()
            ...
            with tabs[2]:
                ...
                if "limites_df" in st.session_state:
                    ...
                    df_limites["Date"] = pd.to_datetime(df_limites["Date"])
                    ...
                    df_limites["Ano"] = df_limites["Date"].dt.year
                    df_limites["Mês"] = df_limites["Date"].dt.month
                    ...
                    st.warning("No limits have been loaded.")
            ...
            with tabs[4]:
                ...
                consumo_completo["Date"] = consumo_completo["Datetime"].dt.date
                dias_unicos = sorted(consumo_completo["Date"].unique())
                            ...
                            if not dados_dia.empty:
                                ...
                                if "limites_df" in st.session_state:
                                    limites_dia_df = st.session_state.limites_df[
                                        st.session_state.limites_df["Date"] == dia.date()
                                        ]
                                    limites_area_dia = [
                                        sum(
            ...
            with tabs[6]:
                ...
                if "limites_df" in st.session_state and "data_selecionada" in st.session_state and "consumo" in st.session_state:
                    ...
                    limites_df["Date"] = pd.to_datetime(limites_df["Date"])
                    limites_mes = limites_df[
                        (limites_df["Date"].dt.month == data_ref.month) &
                        (limites_df["Date"].dt.year == data_ref.year)
                        ]

                    # Consumo máximo previsto
                    consumo_max_mes = limites_mes[colunas_area_produtiva].sum().sum()
                    dias_mes = limites_mes["Date"].dt.date.nunique()
                    ...
                    limites_restantes = limites_mes[limites_mes["Date"].dt.date > data_ref]
                    targets_restantes = limites_restantes[colunas_area_produtiva].sum().sum()
                    adicional_restante = limites_restantes["Date"].dt.date.nunique() * 24 * 13.75
                    ...
                    dias_esperados = set(limites_mes["Date"].dt.date.unique())
                    ...
                    col1.metric("🔋 Maximum consumption forecast for the month (production area)", f"{consumo_max_mes:.2f} kWh")
                    ...
                    targets_ate_hoje = limites_mes[limites_mes["Date"].dt.date <= data_ref][
                        colunas_area_produtiva].sum().sum()
                    adicional_ate_hoje = limites_mes[limites_mes["Date"].dt.date <= data_ref][
                                             "Date"].dt.date.nunique() * 24 * 13.75
                    ...
                    df_consumo["Date"] = pd.to_datetime(df_consumo["Datetime"]).dt.date
                    df_diario = df_consumo.groupby("Date")["Área Produtiva"].sum().reset_index()
                    df_diario["Date"] = pd.to_datetime(df_diario["Date"])

                    # Filtrar mês de referência
                    df_mes = df_diario[
                        (df_diario["Date"].dt.month == data_ref.month) &
                        (df_diario["Date"].dt.year == data_ref.year)
                        ]

                    consumo_ate_hoje = df_mes["Área Produtiva"].sum()
                    dias_consumidos = df_mes["Date"].nunique()
                    ...
                    meta_mensal = df_limites[
                        (df_limites["Date"].dt.month == data_ref.month) &
                        (df_limites["Date"].dt.year == data_ref.year)
                        ]["Meta Horária"].sum()
                    ...
                    datas_unicas = sorted(limites_mes["Date"].dt.date.unique())
                    dados_tabela = []

                    for dia in datas_unicas:
                        limites_dia = limites_mes[limites_mes["Date"].dt.date == dia]
                    ...
                    historico_diario = df_consumo[
                        ...
                        ].groupby("Date")["Área Produtiva"].sum()

                    if len(historico_diario) >= 5:
                        media = historico_diario.mean()
                        desvio = historico_diario.std()
                        dias_futuros = [datetime.strptime(d, "%Y-%m-%d").date() for d in df_tabela["Date"] if
                        ...
                        colunas_area = ["MP&L", "GAHO", "CAG", "SEOB", "EBPC", "PMDC-OFFICE", "OFFICE + CANTEEN",
                                        "TRIM&FINAL"]
                        df_limites["Meta Horária"] = df_limites[colunas_area].sum(axis=1) + 13.75
                        meta_diaria_df = df_limites.groupby("Date")["Meta Horária"].sum().reset_index()
                        ...
                        meta_diaria_df.columns = ["Date", "Meta Horária"]
                        ...
                        meta_total = meta_diaria_df[meta_diaria_df["Date"].isin(datas_relevantes)]["Meta Horária"].sum()
                        ...
                        - **{em_baixa}** simulations indicate upward trend in consumption  
                        ...
                        if 'consumo' in st.session_state:
                            ...
                            df_diario = df.groupby('Date')['Área Produtiva'].sum().reset_index()
                            df_diario['Date'] = pd.to_datetime(df_diario['Date'])
                            serie_historica = pd.Series(df_diario['Área Produtiva'].values, index=df_diario['Date'])
                            ...
                            datas_completas = pd.date_range(start=meta_diaria_df["Date"].min(),
                                                            end=ultimo_dia_mes.date(), freq='D')
                            meta_diaria_df = meta_diaria_df.set_index("Date").reindex(datas_completas).fillna(
                            ...
                            fig.update_layout(title='🔍 Energy Consumption Forecasting: ARIMA vs Monte Carlo',
                                              xaxis_title='Date', yaxis_title='Consumo (kWh)',
                            ...
                            df_consumo = st.session_state.consumo.copy()
                            df_consumo["Date"] = df_consumo["Datetime"].dt.date
                            consumo_diario = df_consumo.groupby("Date")["Área Produtiva"].sum().reset_index()
                            ...
                            df_limites = df_limites[
                                (pd.to_datetime(df_limites["Date"]).dt.month == mes) &
                                (pd.to_datetime(df_limites["Date"]).dt.year == ano)
                                ]
                            consumo_diario = consumo_diario[
                                (pd.to_datetime(consumo_diario["Date"]).dt.month == mes) &
                                (pd.to_datetime(consumo_diario["Date"]).dt.year == ano)
                                ]
                            ...
                            df_plot = meta_diaria_df.merge(consumo_diario, on="Date", how="left")
                            ...
                            mask_passado = df_plot["Date"] <= hoje
                            mask_futuro = df_plot["Date"] > hoje
                            ...
                            fig = go.Figure()
                            fig.add_trace(go.Scatter(
                                x=df_plot["Date"], y=df_plot["Consumo Real"],
                                mode='lines+markers', name='Consumo Real Diário', line=dict(color='blue')
                            ))
                            fig.add_trace(go.Scatter(
                                x=df_plot["Date"], y=df_plot["Meta Original"],
                                mode='lines', name='Meta Original Diária', line=dict(dash='dash', color='black')
                            ))
                            fig.add_trace(go.Scatter(
                                x=df_plot["Date"], y=df_plot["Nova Meta Ajustada"],
                                mode='lines', name='Nova Meta Ajustada', line=dict(dash='dot', color='orange')
                            ))
                            fig.update_layout(
                                title='Daily Consumption of the Productive Area vs Targets (Proportional Distribution)',
                                xaxis_title='Date',
                            ...
                            if saldo_energia >= 0:
                                ...
                                st.success(f"""
                                ✅ To date, there is a positive balance of  **{saldo_energia:,.0f} kWh** of energy..
                                  This allows for approximately **{horas_extras:.1f} hours** of extra air conditioning until the end of this month,
                                which is equivalent to about **{dias_extras:.1f} days** of additional air conditioning.
                                """)
                            ...
                            # Métricas
                            st.markdown("### 📈 Summary of Monthly Goals")
                            col1, col2 = st.columns(2)
                            ...
                            st.subheader("📈 Interactive Forecast with Monte Carlo")
