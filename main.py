...
if dados_colados:
        try:
            ...
            if "limites_diarios" not in st.session_state:
                st.session_state.limites_diarios = pd.DataFrame(columns=["Data", "Medidor", "Limite Diário"])
            ...
            with tabs[1]:
                ...
                for medidor in medidores_disponiveis:
                    ...
                    limite_diario = st.session_state.limites_diarios[
                        (st.session_state.limites_diarios["Data"] == str(data_selecionada)) &
                        (st.session_state.limites_diarios["Medidor"] == medidor)
                    ]["Limite Diário"].values
                    if limite_diario:
                        limite_horario = [limite_diario[0] / 24] * 24
                    else:
                        limite_horario = [5.0] * 24
